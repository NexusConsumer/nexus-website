...
import {
  ...
  ArrowUp, ArrowDown, TrendingUp, Eye, MousePointerClick, MapPin,
  AlertTriangle,
} from 'lucide-react';
  ...
  const fetchAll = useCallback(async () => {
    ...
    results.forEach((result, i) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        switch (key) {
          case 'gscOverview': setGscOverview(data); break;
          case 'gscQueries': setGscQueries(Array.isArray(data) ? data : data?.rows ?? []); break;
          case 'gscPages': setGscPages(Array.isArray(data) ? data : data?.rows ?? []); break;
          case 'ga4Overview': setGa4Overview(data); break;
          case 'trafficSources': setTrafficSources(Array.isArray(data) ? data : data?.rows ?? []); break;
          case 'devices': setDevices(Array.isArray(data) ? data : data?.rows ?? []); break;
        }
      } else {
        ...
        <div className="space-y-8">
          ...
          <section>
            <h2 className="text-lg font-semibold text-stripe-dark mb-4 flex items-center gap-2">
              ...
              {isHe ? 'Google Search Console' : 'Google Search Console'}
            </h2>
            {errors.gscOverview ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <KpiCard
                  label="CTR"
                  value={`${(gscOverview.ctr * 100).toFixed(1)}%`}
                  icon={TrendingUp}
                  color="#f59e0b"
                />
                <KpiCard
                    ...
                    <tbody>
                      {gscQueries.map((q, i) => (
                        <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50">
                          ...
                          <td className="px-4 py-2.5 text-end tabular-nums text-stripe-gray">{(q.ctr * 100).toFixed(1)}%</td>
                    ...
                    <tbody>
                      {gscPages.map((p, i) => (
                        <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50">
                          ...
                          <td className="px-4 py-2.5 text-end tabular-nums text-stripe-gray">{(p.ctr * 100).toFixed(1)}%</td>
          ...
          <section>
            <h2 className="text-lg font-semibold text-stripe-dark mb-4 flex items-center gap-2">
              ...
              {isHe ? 'Google Analytics' : 'Google Analytics'}
            </h2>
            {errors.ga4Overview ? (
              <UnavailableCard isHe={isHe} />
            ) : ga4Overview ? (
              ...
              <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/50">
                        <th className="text-start px-4 py-2.5 font-medium text-stripe-gray">{isHe ? 'מקור' : 'Source'}</th>
                        <th className="text-start px-4 py-2.5 font-medium text-stripe-gray">{isHe ? 'מדיום' : 'Medium'}</th>
                    ...
                    <tbody>
                      {trafficSources.map((s, i) => (
                        <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50">
                          ...
                          <td className="px-4 py-2.5 text-stripe-gray">{s.medium ?? '—'}</td>
                      ...
                      {trafficSources.length === 0 && (
                        <tr><td colSpan={4} className="px-4 py-8 text-center text-stripe-gray text-xs">{isHe ? 'אין נתונים' : 'No data'}</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          ...
                          {d.percentage !== undefined && (
                            <div className="text-xs text-stripe-gray">{(d.percentage * 100).toFixed(1)}%</div>
                          )}
                        </div>
                      </div>
