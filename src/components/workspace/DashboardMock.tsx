...
// ── Nav items matching the real sidebar ───────────────────────────────────────
const NAV: { icon: keyof typeof ICONS; label: string; active?: boolean }[] = [
  { icon: 'dashboard',  label: 'Dashboard',                     active: true },
  { icon: 'people',     label: 'Users' },
  { icon: 'gift',       label: 'נקודות ומתנות' },
  { icon: 'tag',        label: 'הטבות ושיתופי פעולה' },
  { icon: 'article',    label: 'Content' },
  { icon: 'chart',      label: 'דוחות' },
  { icon: 'campaign',   label: 'שיווק' },
  { icon: 'sync',       label: 'עדכונים' },
  { icon: 'chat',       label: "צ'אט" },
  { icon: 'code',       label: 'Developer Tools' },
  { icon: 'settings',   label: 'Settings' },
];

// ── Stats matching mockData ───────────────────────────────────────────────────
const STATS = [
...
const RECENT_USERS = [
  { initial: 'J', name: 'John Smith',    email: 'john@example.com',  status: 'active' },
  { initial: 'S', name: 'Sarah Johnson', email: 'sarah@example.com', status: 'active' },
  { initial: 'M', name: 'Mike Wilson',   email: 'mike@example.com',  status: 'active' },
];

const RECENT_CONTENT = [
  { emoji: '📄', title: 'Welcome to Our Platform',  author: 'John Smith',    status: 'published' },
  { emoji: '📝', title: 'Getting Started Guide',     author: 'Sarah Johnson', status: 'published' },
  { emoji: '📝', title: 'Product Announcement',      author: 'John Smith',    status: 'draft' },
];

// ─────────────────────────────────────────────────────────────────────────────
export default function DashboardMock() {
  return (
    // dir="rtl" mirrors the actual app: sidebar on the RIGHT, content on the LEFT
    <div dir="rtl" className="absolute inset-0 flex flex-col overflow-hidden select-none pointer-events-none" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>

      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <header className="bg-white border-b border-slate-200 h-[73px] flex items-center px-6 shrink-0 z-10">

        {/* Right side (flex-start in RTL): user controls */}
        <div className="flex items-center gap-3">
          {/* Avatar */}
          <div className="flex items-center gap-2 ps-4 border-s border-slate-200">
          ...
          {/* Headset */}
          <div className="p-2">
            <Icon d={ICONS.headset} size={20} color="#64748b" />
          </div>
          {/* Group */}
          <div className="p-2">
            <Icon d={ICONS.group} size={20} color="#64748b" />
          </div>
          {/* Upgrade */}
          ...
          {/* Language */}
          <div className="px-2 py-1 text-[12px] text-slate-500 border border-slate-200 rounded-md">EN / HE</div>
        </div>

        {/* Left side (ms-auto → pushes to the left in RTL): search + logo */}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 bg-white border-l border-slate-200 shrink-0 flex flex-col min-h-0">

          {/* Project Switcher */}
          <div className="px-3 py-3 border-b border-slate-200">
            <div className="flex items-center gap-2 px-3 py-2.5 bg-slate-100 rounded-xl">
              <Icon d={ICONS.web} size={18} color="#0066cc" />
              <span className="text-[13px] font-medium text-slate-900 flex-1 truncate text-start">Nexus-Online</span>
          ...
          <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-hidden">
            {NAV.map((item) => (
              ...
              >
                ...
                {item.icon === 'code' && (
                  <Icon d={ICONS.chevron} size={16} color="#9ca3af" />
                )}
              </div>
            ))}
          </nav>
          ...
          <div className="px-3 pb-3">
            <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[#0066cc] text-[11px] font-semibold">Upgrade</span>
              </div>
              <p className="text-[11px] text-slate-600 mb-2 leading-snug">
                שדרג לפרימיום וקבל גישה לכל התכונות
              </p>
        ...
        <main className="flex-1 bg-[#f3f7fa] overflow-hidden p-6">
          ...
          {/* Greeting banner — afternoon style matching Dashboard.tsx */}
          <div className="w-full mb-5 rounded-xl px-6 py-4 border-2 border-blue-200/50 overflow-hidden relative"
            ...
            <div className="flex items-center justify-center gap-3">
              <h1 className="text-[1.35rem] font-bold text-blue-900 flex items-center gap-2">
                ...
                <span>אחר צהריים טובים, Admin</span>
              </h1>
            </div>
            <p className="text-center text-[13px] text-blue-700 font-medium mt-1">שמחים לראות אותך שוב</p>
          </div>

          {/* Stats grid — 4 columns matching Dashboard.css .stats-grid */}
          ...
          {/* Dashboard grid — 2 columns matching Dashboard.css .dashboard-grid */}
          <div className="grid grid-cols-2 gap-5">

            {/* Recent Users */}
            <div className="bg-white rounded-lg p-5" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.10)' }}>
              <h3 className="text-[1.1rem] font-medium text-[#1f2937] mb-4">Recent Users</h3>
              <ul>
            ...
            {/* Recent Content */}
            <div className="bg-white rounded-lg p-5" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.10)' }}>
              <h3 className="text-[1.1rem] font-medium text-[#1f2937] mb-4">Recent Content</h3>
              <ul>
                {RECENT_CONTENT.map((c, i) => (
                  <li key={i} className="flex items-center gap-3 py-3 border-b border-[#e5e7eb] last:border-0">
                    <div className="w-9 h-9 flex items-center justify-center text-xl shrink-0">{c.emoji}</div>
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-[#1f2937] text-[14px] truncate">{c.title}</div>
                      <div className="text-[#6b7280] text-[0.85rem]">{c.author}</div>
                    </div>
